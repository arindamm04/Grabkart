 ...
 */
function buildGrabkartTextLayer({ w, h }) {
    ...
    return `l-text,i-Grabkart,fs-${fs},co-FFFFFF,bg-0F172A90,pa-8_12,lx-N14,ly-14,lap-top_right,l-end`;
}
...
function buildTrSegment({ w, h, q = 80, f = "auto", crop, watermark = false }) {
    ...
    return `${base}:${buildGrabkartTextLayer({ w, h })}`;
}
...
/**
 * Same optimizations as {@link imageKitOptimizedUrl} plus Grabkart text overlay (for share/download).
